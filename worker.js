onmessage = function (e) {
  const { input, baseTargets, maxTry, excluded } = e.data;
  const result = findOptimalAdjustment(input, baseTargets, maxTry, new Set(excluded));
  postMessage(result || null);
};

const denominations = [10000, 5000, 1000, 500, 100, 50, 10, 5, 1];

function findOptimalAdjustment(input, baseTargets, maxTry, excludeSet) {
  let best = null;
    ...
    for (let a = targetAmount; a >= 0; a--) {
      if (dp[a] !== null) {
        for (let k = 1; k <= count; k++) {
          ...
          if (dp[newAmount] === null || totalCoins(newCombo) < totalCoins(dp[newAmount])) {
